							...
							<thead>
                                ...
                                <tbody>
                                    @foreach ($eventos as $evento)
                                        <tr class="odd">
                                            <td>{{ $evento->titulo ?? 'NO HAY REGISTRO' }}</td>
                                            <td>{{ $evento->descripcion ?? 'NO HAY REGISTRO ' }}</td>
                                            <td>{{ $evento->fecha ?? 'NO HAY REGISTRO' }}</td>
                                            <td>{{ "Desde:".$evento->hora_desde ?? 'NO HAY REGISTRO' .' Hasta:'.$evento->hora_fin }}</td>
                                            <td style="width: 13%">
                                                <button class="btn btn-primary">Resultado</button>
                                                <a href="{{ route('posteventquestions.index',$evento) }}" class="btn btn-primary">Preguntas</button>
                                                <a class="btn btn-danger" href="{{route('eventos.eliminar',$evento->id)}}" >Eliminar</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
